...
#             self._nsource += 1
    ...
    def _normalize(self, source, ext):
        ...
        # Python
            ...
            source = ''.join(norm_lines)
        # Ruby, GEMFILE
        elif ext==common.FileExt.Ruby or ext==common.FileExt.GEMFILE:
            norm_lines = []
        ...
        # JS, SCALA, CPP, Kotlin, gradle, C#, VUE, JSX
        elif ext==common.FileExt.JavaScript or \
            ext==common.FileExt.Scala or \
            ext==common.FileExt.CPP or ext==common.FileExt.Kotlin or \
            ext==common.FileExt.gradle or ext==common.FileExt.CSHARP or \
            ext==common.FileExt.VUE or ext==common.FileExt.REACT :
            norm_lines = []
            for c in common.js_regex.finditer(source):
                if c.group('noncomment'):
                    norm_lines.append(c.group('noncomment'))
                elif c.group('multilinecomment'):
                    newlines_cnt = c.group('multilinecomment').count('\n')
            ...
            source = ''.join(norm_lines)
        # xml
        elif ext==common.FileExt.Xml:
            norm_lines = []
            for c in common.xml_regex.finditer(source):
                if c.group('noncomment'):
                    norm_lines.append(c.group('noncomment'))
                elif c.group('multilinecomment'):
                    newlines_cnt = c.group('multilinecomment').count('\n')
...
#                 print('self._bit_vector[h] = ', self._bit_vector[h])
    ...
    def _get_file_type(self, sub_type):
        ...
        elif sub_type == 'x-bash':
            magic_ext = common.FileExt.Bash
        ...
        elif sub_type == 'x-gemfile':
            magic_ext = common.FileExt.GEMFILE
        elif sub_type == 'x-javascript':
            magic_ext = common.FileExt.JavaScript
        elif sub_type == 'x-typescript':
            magic_ext = common.FileExt.TypeScript
        elif sub_type == 'x-cpp':
            magic_ext = common.FileExt.CPP
        elif sub_type == 'x-csharp':
            magic_ext = common.FileExt.CSHARP
        elif sub_type == 'x-xml':
            magic_ext = common.FileExt.Xml
        else:
